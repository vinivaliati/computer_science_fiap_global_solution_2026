...
A Missão GB simula uma estação orbital, chamada Estação GB-1 (Global solution 1). O objetivo do sistema desenvolvido é monitorar continuamente os dados de telemetria da estação, identificar situações de risco, gerar alertas automáticos e recomendar ações corretivas.

Em órbita, a comunicação com a Terra pode ser interrompida, sensores podem falhar e recursos como energia e oxigênio são limitados. O sistema precisa ser capaz de detectar essas situações e agir com base em regras lógicas bem definidas.
...
A Estação GB-1 opera com seis módulos críticos que não podem falhar sem colocar a missão em risco. São eles: suporte a vida (oxigênio), energia (painéis solares), comunicação com a Terra, habitat (módulo principal), controle de pressão atmosférica e combustível para manobras orbitais.
...
Utilizadas para armazenar séries temporais do CSV: geração solar, consumo, reserva de bateria, radiação e temperatura interna. Listas são a estrutura natural para dados sequenciais permitem acesso por índice, fatiamento e iteração, que são as operações necessárias para análise de tendência e previsão.
...
|----------|--------|--------|---------|
| Bateria | ≥ 25% | 15–24% | < 15% |
| Radiação | ≤ 0.8 mSv/h | 0.8–1.0 mSv/h | > 1.0 mSv/h |
...
A variável escolhida para previsão foi a **reserva de bateria** (`reserva_bateria_pct`).

### 6.2 Metodologia

O sistema aplica dois métodos em sequência:
...
Um período muito curto (1-2 dias) captura apenas as oscilações recentes e é sensível a eventos pontuais. Um período muito longo dilui o impacto de mudanças recentes.
...
--- ANALISE DE INCONSISTENCIA ---
  Linha 52 | 2026-01-09 08:00
  Geracao solar registrada : 980.0 kWh
  Capacidade maxima realista: ~200 kWh (horario de pico)
  Anomalia: valor 5x acima do maximo esperado
  Causa provavel: falha ou descalibracao do sensor de geracao solar
  [ALERTA] Inconsistencia detectada -> recalibracao de sensor iniciada
